"""Geo resolution: city/airport name -> IATA code + lat/lng.

Duffel needs IATA codes for flights and lat/lng for hotel searches, so every
agent resolves names through this module. Resolution order:

1. Built-in dictionary (fast, offline, covers common demo cities).
2. If the input already looks like an IATA code (3 uppercase letters), use it
   directly and look up coordinates from the dictionary.
"""
from __future__ import annotations

from typing import Any

# name -> {iata, lat, lng, country}
...
# IATA code -> coordinates (for direct code input like "LHR").
IATA_COORDS: dict[str, dict[str, float]] = {
...
def resolve_city(name: str) -> dict[str, Any] | None:
    """Resolve a city name (or IATA code) to {iata, lat, lng, name}."""
    if not name:
        return None
    ...
    # Direct IATA code input (e.g. "LHR", "CDG").
    code = key.upper()
    if len(code) == 3 and code.isalpha():
        coords = IATA_COORDS.get(code)
        if coords:
            return {
                "iata": code,
                "lat": coords["lat"],
    ...
    # Fuzzy: match against city names containing the keyword.
...
def resolve_airport(name: str) -> dict[str, Any] | None:
    """Resolve an airport/city name to an IATA airport code."""
    if not name:
        return None
    key = name.strip().lower()
    code = key.upper()
    if len(code) == 3 and code.isalpha() and code in IATA_COORDS:
        coords = IATA_COORDS[code]
        return {"iata": code, "lat": coords["lat"], "lng": coords["lng"], "name": code}
    city = resolve_city(name)
    if city:
        # Prefer a concrete airport code when the city has one.
        concrete = {
        ...
        iata = concrete.get(city["iata"], city["iata"])
